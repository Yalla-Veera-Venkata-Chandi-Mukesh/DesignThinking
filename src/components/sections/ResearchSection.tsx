  ...
  return (
    <section id="research" className="py-32">
      ...
      <div ref={ref} className="container-main pt-32">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ ease }} className="section-label">Empathy Phase</motion.div>
        <motion.h2 initial={{ opacity: 0, y: 24 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ delay: 0.1, ease }} className="section-heading">Research & Survey <em>Insights</em></motion.h2>
        ...
        <div className="space-y-3">
          {findings.map((f, i) => (
            <motion.div key={i} initial={{ opacity: 0, y: 16 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ delay: 0.3 + i * 0.08, ease }} className="flex gap-4 items-start p-5 rounded-2xl" style={{ background: "rgba(255, 255, 255, 0.02)", border: "1px solid rgba(255,255,255,0.04)" }}>
              <div className="w-[2px] shrink-0 rounded-full self-stretch" style={{ background: i % 2 === 0 ? "#34D399" : "#2DD4BF", opacity: 0.7 }} />
              <p className="text-[0.92rem] text-[#9CA3AF] leading-relaxed">{f}</p>
            </motion.div>
          ))}
        </div>

        {/* AI Problem Callout */}
        <motion.div initial={{ opacity: 0, y: 24 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ delay: 0.7, ease }} className="card-glow p-8 md:p-10 mt-8">
          <p className="text-[0.88rem] font-semibold mb-3" style={{ color: "#10B981" }}>🤖 The AI Amplification Problem</p>
